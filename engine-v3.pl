...
% pkt(DataName, val(Msg), KeyInfo, h(Data), s(Signature))
...
fetchTrustedKey(DataName, Key, TrustContextIn, TrustContextOut) :-
  ...
  TrustContextOut = trustCtx('schematized', [Key | KeyList], Schema).
...
hmac :-
  ...
  Pkt = pkt('/a/data/name', val(1000), key(name('/key/1'),
            id(102), _), h(val(1000)), s(1011)),
  % case 2: pkt contains no key information - we try out all known keys
  % Pkt = pkt('/a/data/name', val(1000), _, h(val(1000)), s(1022)),
  myHmacTrust(Ctx),
...
hier :-
  ...
  Msg = val(2000),
  Pkt = pkt('/a/data/name', Msg, key(name('/key/10'), _, _), h(Msg), s(2001)),
  myHierTrust(Ctx),
  isValidPkt(Pkt, Ctx, _).
